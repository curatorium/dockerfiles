...
### 2026-07
1. Split the single multi-stage `Dockerfile` into per-role directories -- each role owns `<role>/Dockerfile`, `<role>/Stewardfile`, `<role>/files/` (php-base/php-qa also `mods-available/`)
1. Root `Dockerfile` is now GENERATED by `./generate-dockerfile` (concatenates the per-role Dockerfiles, rewrites each published `FROM` to its local build stage, role-prefixes relative `COPY` sources) -- never hand-edit it
1. Replaced the 8 `files/usr/local/bin/add-*` bash installers with steward `Stewardfile`s (primitives: `apt`/`key`/`src`/`deb`/`bin`/`tar`/`zip`/`ext`/`npm`/`composer`, `defer` only where no primitive fits); steward is `ADD`ed in base's Dockerfile then runs each role's Stewardfile
1. Base is now `debian:bookworm-slim` (was `nginx:bookworm`); nginx moved to `php-base` (apt + `gettext-base` for envsubst)
1. Two lineages off `base`: `base → ci → az-ci` (ci is now PHP-free; azure split out into `az-ci`) and `base → php-base → php-qa → php-fs`
1. New repo naming: `base`/`ci`/`az-ci` publish to `curatorium/<role>` (no PHP version); `php-base`/`php-qa`/`php-fs` publish to `curatorium/php-<PHPVS>:<base|qa|fs>`
1. `add-debug`/`add-forensics` kept as runtime scripts in `base/files/`
1. Split the monolithic `tests/image.test` into per-role suites `tests/<role>.test`
1. `service install` forwards a daemonized command's arguments as an array -- glob metacharacters (ex.: a `*` routing key) now reach the command verbatim instead of expanding against the working dir
1. PHP 8.0 builds again -- `easy-config` ignores the twig security advisories that block every twig old enough for 8.0
...
1. Added HTTPie to the ci image
1. Added `phpstan/phpstan-deprecation-rules` to the qa image
1. The NewRelic daemon can be installed separately from the agent
1. NewRelic CLI default profile rendered at login (alongside credentials)
1. Quieter NGINX boot (`NGINX_ENTRYPOINT_QUIET_LOGS`)
